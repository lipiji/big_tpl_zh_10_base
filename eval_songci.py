...
ds = []
di = 0
with open("./model/dev.txt", "r") as f:
    for line in f:
        line = line.strip()
        if line:
            # for song ci
            fs = line.split("<s2>")
            author, topic = fs[0].split("<s1>")
            sents = fs[1].split("</s>")
            ss = []
            for sent in sents:
                ss += [w for w in sent] + ["</s>"]

            ds.append(' '.join([w for w in topic]  + ["<eoc>"] + ss))
            di += 1
print(len(ds))
...
while idx < len(ds):
    ...
    nll, ppl, bsz = lm_model.ppl(ys_truth, ys_inp, ys_tpl, ys_seg, ys_pos, msk)
